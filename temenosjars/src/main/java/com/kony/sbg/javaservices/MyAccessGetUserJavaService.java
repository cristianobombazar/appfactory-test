...
public class MyAccessGetUserJavaService  implements JavaService2 {

	private static final Logger LOG = Logger.getLogger(MyAccessGetUserJavaService.class);

	@Override
	public Object invoke(String methodID, Object[] inputArray, DataControllerRequest dcRequest,
			DataControllerResponse dcResponse) throws Exception {
		LOG.debug("Enty --> MyAccessGetUserJavaService");
		Result result = new Result();
		try {
			PaymentFeedbackResource resource = DBPAPIAbstractFactoryImpl.getResource(PaymentFeedbackResource.class);		
			result = resource.fetchMyAccessGetUsers(methodID, inputArray, dcRequest, dcResponse);
		} catch (Exception exp) {
			LOG.error("Exception occured in MyAccessGetUserJavaService: ", exp);
			LOG.error("Exception occured in MyAccessGetUserJavaService: " + exp.getStackTrace());
