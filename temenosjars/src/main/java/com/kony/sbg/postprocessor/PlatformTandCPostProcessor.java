...
public class PlatformTandCPostProcessor implements DataPostProcessor2 {

    private static final Logger LOG = LogManager.getLogger(PlatformTandCPostProcessor.class);
    ...
    public Object execute(Result result, DataControllerRequest dataControllerRequest,
        ...
        try {
            ...
            if (SBGConstants.TRUE.equalsIgnoreCase(isDesignedPerson) || "Y".equalsIgnoreCase(DESGINDICATOR)) {

                eventSubType = SBGConstants.TRUE.equalsIgnoreCase(isEBTCaccepted) ? "TERMS_AND_CONDITIONS_ACCEPT"
                        : "TERMS_AND_CONDITIONS_DENY";

            } else {
                eventSubType = "TERMS_AND_CONDITIONS_ACKNOWLEDGE";
            }

            _OlbAuditLogsClass(dataControllerRequest, dataControllerResponse, result, "TERMS_AND_CONDITIONS",
                    eventSubType,
                    "services/data/v1/SbgEBTCObjects/operations/EBTCObject/updateCustomerEBTCdata", "success",
                    customParams);
