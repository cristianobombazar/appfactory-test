...
public class RefDataCacheHelper {
	
	private static final Logger logger 	= LogManager.getLogger(RefDataCacheHelper.class);
	...
	public	static String getRefDataByKey(DataControllerRequest request, String country, String currency) {
		...
		String REFDATACACHE = "REFDATA-" + country + "-" + currency;
		logger.debug("RefDataCacheHelper.getRefDataByKey() ---> START ===> ctrycurr: "+REFDATACACHE);
		...
		try {
			...
			logger.error("RefDataCacheHelper.getRefDataByKey ---> 1. EXCEPTION: "+e.getMessage());
		} 
		...
		if(SBGCommonUtils.isStringEmpty(retval)) {
			...
			logger.debug("RefDataCacheHelper.getRefDataByKey() ---> data4mDB: "+data4mDB);
			...
			if(data4mDB == null) {
    			...
    			logger.debug("RefDataCacheHelper.getRefDataByKey() ---> latestRefData: "+latestRefData);
    			
    			if(isValidRefData(latestRefData)) {
    				//if the response is valid then updating the latest response in db and cache
    				logger.debug("RefDataCacheHelper.getRefDataByKey() ---> inserting data in DB and updating cache");
    				...
    				logger.error("RefDataCacheHelper.getRefDataByKey() ---> REFERENCE DATA API FAILED ");
    			}
			} else {
				String updatedAt = data4mDB.get("time");
				String refData	 = data4mDB.get("data");
				if(canUpdateRefData(updatedAt)) {
					//if the data saved in DB has crossed more than 24 hours then pulling latest from sbg
					logger.debug("RefDataCacheHelper.getRefDataByKey() ---> fetched data from DB and the data is expired");
	    			String latestRefData = getRefData4mSBG(request, country, currency);
	    			if(isValidRefData(latestRefData)) {
	    				//latest data is received from sbg, updating latest data at all placess
	    				logger.debug("RefDataCacheHelper.getRefDataByKey() ---> updating data in DB and updating cache");
	    				retval = latestRefData;		    			
	    				...
	    				logger.debug("RefDataCacheHelper.getRefDataByKey() ---> fetching data from sbg failed. using existing data and updating cache");
	    				//in case if the data is not returned from SBG then we are returning null/empty response.
					...
					//using the data from DB and just updating the value in Cache to extend time of expiry
					logger.debug("RefDataCacheHelper.getRefDataByKey() ---> data from DB is valid. Using the same and updating cache");
					retval = new String(Base64.getDecoder().decode(refData));
	    			saveRefDataInCache(request, REFDATACACHE, retval);
				}
			}			
		...
		logger.debug("RefDataCacheHelper.getRefDataByKey() ---> END ===> "+retval);
		return retval;
	}
	
	private	static boolean isValidRefData(String refData) {
		logger.debug("RefDataCacheHelper.isValidRefData() ---> refData: "+refData);
		...
		try {
			...
			logger.debug("RefDataCacheHelper.isValidRefData() ---> daysArray: "+daysArray);
			
			return daysArray != null && daysArray.length() > 0;
			
		}catch(Exception e) {
			logger.error("RefDataCacheHelper.isValidRefData() ---> EXCEPTION: "+e.getMessage());
			return false;
		}
	}

	public	static Map<String, Object> constructHeaderParams(DataControllerRequest request, Map<String, Object> headerParams, String country, String currency)
			throws Exception {
		ConfigurableParametersHelper configurableParametersHelper = request.getServicesManager().getConfigurableParametersHelper();
		...
		if(StringUtils.isNotBlank(request.getParameter("paymentType"))){
			if(SbgURLConstants.IAT_PAYMENT_TYPE_DOM.equalsIgnoreCase(request.getParameter("paymentType"))){
				sbgHeaderApiProductCode = SBGConstants.IAT_PRODUCT_CODE;
			} else if(SbgURLConstants.IAT_PAYMENT_TYPE_FX.equalsIgnoreCase(request.getParameter("paymentType"))){
				sbgHeaderApiProductCode = SBGConstants.IATFX_PRODUCT_CODE;
			}
		}	
		...
		logger.info("[RefDataCacheHelper.constructHeaderParams]: " + headerParams.toString());
		return headerParams;
	}
	...
	public static Map<String, String> getRefDataFromDB(DataControllerRequest request, String ctrycurr) {
		...
		try {
	    	...
	    	//logger.debug("RefDataCacheHelper.getRefDataFromDB ===> filter: "+filter);
	    	...
	    	logger.debug("RefDataCacheHelper.getRefDataFromDB() ---> result: "+ResultToJSON.convert(result));
			...
			logger.error("RefDataCacheHelper.getRefDataFromDB ===> EXCEPTION: "+e.getMessage());
		}
		logger.debug("RefDataCacheHelper.getRefDataFromDB() ---> retval: "+retval);
		return retval;
	}
	
	public static void insertDataInDB(DataControllerRequest request, String ctrycurr, String data) {
		logger.debug("RefDataCacheHelper.insertDataInDB() ---> ctrycurr: "+ctrycurr +"; data: "+data);
		try {
			HashMap<String, Object> svcHeaders = new HashMap<String, Object>();
			HashMap<String, Object> svcParams = new HashMap<String, Object>();
	    	...
	    	logger.debug("RefDataCacheHelper.insertDataInDB ===> Insert result: "+ResultToJSON.convert(result));
		}catch(Exception e) {
			logger.error("RefDataCacheHelper.insertDataInDB ===> EXCEPTION: "+e.getMessage());
		}
	}
	
	public static void updateDataInDB(DataControllerRequest request, String ctrycurr, String data) {
		logger.debug("RefDataCacheHelper.updateDataInDB() ---> ctrycurr: "+ctrycurr +"; data: "+data);
		try {
			HashMap<String, Object> svcHeaders = new HashMap<String, Object>();
			HashMap<String, Object> svcParams = new HashMap<String, Object>();
			svcParams.put("refdata", Base64.getEncoder().encodeToString(data.getBytes()));
	    	...
	    	logger.debug("RefDataCacheHelper.updateDataInDB ===> Update result: "+ResultToJSON.convert(result));
		}catch(Exception e) {
			logger.error("RefDataCacheHelper.updateDataInDB ===> EXCEPTION: "+e.getMessage());
		}
	}

	public static void saveRefDataInCache(DataControllerRequest request, String ctrycurr, String data) {
		logger.error("RefDataCacheHelper.saveRefDataInCache ===> ctrycurr: "+ctrycurr+"; data: "+data);
		try {
			ResultCache resultCache = request.getServicesManager().getResultCache();
			resultCache.insertIntoCache(ctrycurr, data, EXPIRYINSECS);			
		} catch (Exception e) {
			logger.error("RefDataCacheHelper.saveRefDataInCache ===> EXCEPTION: "+e.getMessage());
		}
		logger.error("RefDataCacheHelper.saveRefDataInCache ===> END");
	}
	...
	private	static String getRefData4mSBG(DataControllerRequest request, String country, String currency) {
		logger.error("RefDataCacheHelper.getRefData4mSBG ===> START");
		try {
			String serviceName 		= SBGConstants.REFDATA_ORCHSEQ;
			String operationName 	= SBGConstants.REFDATA_ORCHSEQOPR;
			
		//	String serviceName 		= SBGConstants.REFDATA_ORCHESTRATION;
		//	String operationName 	= SBGConstants.REFDATA_ORCH_OPERATION;
			...
			String refDataResponse = sbgInternationalFundTransferBackendDelegateExtn.callRefDataOrchService(
					headerParams, requestParameters, serviceName, operationName);
			
			logger.error("RefDataCacheHelper.getRefData4mSBG ===> refDataResponse: "+refDataResponse);
			return refDataResponse;
		}catch(Exception e) {
			logger.error("RefDataCacheHelper.getRefData4mSBG ===> EXCEPTION: "+e.getMessage());
		}
		return null;
	}
}
